        ...
        public Patient ConvertCSVFormatToEntity(string entityCSVFormat)
        {
            try
            {
             ...
             //                  select ct.DateTimeFormat.GetAllDateTimePatterns()).SelectMany((x) => x).ToArray();
                ...
                Patient patient = new Patient(long.Parse(tokens[0]), tokens[1], tokens[2], tokens[3], tokens[4], tokens[5], DateTime.ParseExact(tokens[6], "dd.MM.yyyy", CultureInfo.InvariantCulture)/*DateTime.ParseExact(tokens[6], formats, CultureInfo.InvariantCulture, DateTimeStyles.AssumeLocal)*/, null, tokens[8], tokens[9], null);
