   ...
   public class ArticleCSVConverter : ICSVConverter<Article>

    {
        ...
        public Article ConvertCSVFormatToEntity(string entityCSVFormat)
        {
            ...
            return new Article(
                long.Parse(tokens[0]),
               /*(DateTime)DateTime.Parse(tokens[1])*/ DateTime.ParseExact(tokens[1], "dd.MM.yyyy", CultureInfo.InvariantCulture), new Doctor(long.Parse(tokens[2])), tokens[3], tokens[4]);
