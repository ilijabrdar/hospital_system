    ...
    class DoctorCSVConverter : ICSVConverter<Doctor>
    {
        ...
        public Doctor ConvertCSVFormatToEntity(string entityCSVFormat)
        {
           string[] tokens = entityCSVFormat.Split(_delimiter.ToCharArray());
            Doctor doct = new Doctor(long.Parse(tokens[0]), tokens[1], tokens[2], tokens[3], tokens[4], tokens[5], DateTime.Parse(tokens[6]), new Address(long.Parse(tokens[7])), tokens[8], tokens[9],Image.FromFile(tokens[10]), new Specialty(long.Parse(tokens[11])));
