    ...
    public class SecretaryCSVConverter : ICSVConverter<Secretary>
    {
        ...
        public Secretary ConvertCSVFormatToEntity(string entityCSVFormat)
        {
            ...
            return new Secretary(long.Parse(tokens[0]), 
                ...
                tokens[4], tokens[5], tokens[6], tokens[7], tokens[8], DateTime.ParseExact(tokens[9], "dd.MM.yyyy", CultureInfo.InvariantCulture), new Address(long.Parse(tokens[10]), long.Parse(tokens[11]), long.Parse(tokens[12])), Boolean.Parse(tokens[13]));
        }
